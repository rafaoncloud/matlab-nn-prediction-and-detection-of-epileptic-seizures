function [prediction, detection]  = test_deep_network(clf_type, trained_net, ...
    P_test, T_test)
    ...
    T_pred = classify(trained_net, P_test, ...
        'MiniBatchSize', miniBatchSize);
    
    T_pred_matrix = zeros(length(T_pred), 4);
    for i = 1 : length(T_pred)
        T_pred_matrix(i, T_pred(i)) = 1;
    end
    
    T_pred = [];
    T_pred = T_pred_matrix';    
    %% Same as test_network after the previous steps
